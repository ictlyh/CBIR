...
int main()
{
	...
	// 构造查询图像
	char queryImage[] = "E:\\images\\airplane.jpg";
	Image query(queryImage);
	query.calcFeature();

	// 构造查询器
	Searcher searcher;
	list<string> result = searcher.search(query, imageLib);
	// 根据用户反馈进行二次查询
	list<string> feedback;
	list<string> reResult = searcher.reSearch(query, feedback,imageLib);
	return 0;
